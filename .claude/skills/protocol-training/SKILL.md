...
| [`TD-025`](decisions/TD-025-a-deficit-is-reported-never-repaid.md) | A deficit is reported and never repaid, because a queue **delays** volume rather than losing it — what is short is the rate, not the dose. Converts the systematic per-muscle starvation of a regenerated week into a uniform rate deficit. Repaying would add sets above target to a user who just showed less capacity than they declared | active | `volume-progression-across-a-block` (contested), `weekly-set-volume-for-hypertrophy` (settled), `cold-start-first-block` (thin) |
